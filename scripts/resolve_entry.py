...
from cinescore_ai.resolve import RealResolveAdapter


try:
    resolve  # type: ignore[name-defined]
except NameError as exc:  # pragma: no cover - only triggered outside Resolve
    raise RuntimeError(
        "The global 'resolve' object is only available when this script is started from DaVinci Resolve."
    ) from exc


WINDOW = launch_app(resolve_adapter=RealResolveAdapter(resolve), block=True)  # type: ignore[name-defined]
